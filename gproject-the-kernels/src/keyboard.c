...
//                kernel_log_trace("keyboard: CAPS released");
        ...
        default:
            ...
            // Check for navigation keys after mapping
            if (key_pressed) {
                switch (c) {
                    case KEY_HOME:
                    case 0xFFFFFFE0: // home key scancade
                        kernel_log_trace("keyboard: HOME key action");
                        tty_scroll_top();
                        return KEY_NULL;
                    case KEY_END:
                    case 0xFFFFFFE1: // end key scancade
                        kernel_log_trace("keyboard: END key action");
                        tty_scroll_bottom();
                        return KEY_NULL;
                    case KEY_UP:
                    case 0xFFFFFFE6: // page up scancade
                        kernel_log_trace("keyboard: PAGE UP key action");
                        tty_scroll_up();
                        return KEY_NULL;
                    case KEY_DOWN:
                    case 0xFFFFFFE7: // page down scancade
                        kernel_log_trace("keyboard: PAGE DOWN key action");
                        tty_scroll_down();
                        return KEY_NULL;
                    default:
                        break;
